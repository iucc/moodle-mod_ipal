...
function ipal_count_question_codes($questioncode) {// What questions and what responses were given each time question sent.
    ...
    foreach ($answers as $answer) {
        $label = $answer->answer;
        if (preg_match("/\<span\ dir\=[\'\"]rtl[\"\']\>(.*?)\<\/span\>/i", $label, $matches)) {
            $rtltext = strrev($matches[1]);
            $label = preg_replace("/$matches[1]/", $rtltext, $label);
        }
        $labels .= "&x[$n]=".urlencode(substr(strip_tags($label), 0, 40));
